...
| 工具 | 作用 | 权限 |
|---|---|---|
| `cua_status` | 引擎/权限状态与修复指引，`request:true` 触发系统授权弹窗（macOS） | 无 |
...
- 授权按 **responsible process** 归属。引擎作为子进程从 harness 继承宿主应用的授权，因此要在系统设置里给宿主应用打勾，而不是给 `cua-engine`。
- 修改授权后必须**退出并重启**宿主应用才生效。
