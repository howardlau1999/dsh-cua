...
public sealed partial class WinHost
{
    ...
    public JsonObject Screenshot(Params parameters)
    {
        ...
        var region = ApplyRegion(parameters, target);
    ...
    /// </remarks>
    private static Native.RECT ApplyRegion(Params parameters, CaptureTarget target)
    {
        var keys = new[] { "x", "y", "width", "height" };
        var present = keys.Where(parameters.Has).ToArray();
        if (present.Length == 0) return target.Region;
        if (present.Length != 4)
        {
        ...
        if (!target.IsWindowOrDisplay) return requested;
