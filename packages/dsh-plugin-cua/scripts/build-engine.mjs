...
function installFile(built, name) {
  ...
  // failure here.
  //
  // This has to run before signing, which rewrites the file and would make the
  // comparison fail for a reason that is not a stale build.
  const builtSize = statSync(built).size
  const installedSize = statSync(outputPath).size
  if (builtSize !== installedSize || !readFileSync(built).equals(readFileSync(outputPath))) {
    process.stderr.write(
      `cua-engine: ${outputPath} does not match the binary just built at ${built}.\n`
        + (name.endsWith('.exe')
          ? 'Re-run the build; if it happens again, delete native/cua-engine-win/obj and try once more.\n'
