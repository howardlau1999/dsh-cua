...
if (status.screenRecording && !status.sessionLocked) {
  ...
  check(shot.scale > 0, 'cua_screenshot reports pixels per screen unit', String(shot.scale))
  // The mapping the guidance hands the model has to hold for the image it was
  // actually given: region + pixel/scale must land back inside the region.
  check(
    Math.abs(shot.pixelWidth / shot.scale - shot.region[2]) < 1.5,
    'the reported scale describes the delivered image',
    `${String(shot.pixelWidth)}px / ${String(shot.scale)} vs region ${String(shot.region[2])}`,
  )
